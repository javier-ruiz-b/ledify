...
void LayerController::addFadeLayer(QSharedPointer<Layer> from, QSharedPointer<Layer> to, uint16_t myIndex, FadeLayer::Interpolator interpolator, uint16_t startDelayMs, uint16_t durationMs) {
...
void LayerController::addFadeLayer(uint16_t myIndex, uint16_t fromIndex, uint16_t toIndex, FadeLayer::Interpolator interpolator, uint16_t startDelayMs, uint16_t durationMs) {
    addFadeLayer(m_indexedLayers.take(fromIndex),
                 m_indexedLayers.take(toIndex),
                 myIndex, interpolator, startDelayMs, durationMs);
}

uint16_t LayerController::addFadeLayer(uint16_t indexFrom, uint16_t indexTo, FadeLayer::Interpolator interpolator, uint16_t startDelayMs, uint16_t durationMs) {
    uint16_t index = getFreeIndex();
    addFadeLayer(index, indexFrom, indexTo, interpolator, startDelayMs, durationMs);
    return index;
}

void LayerController::addFadeLayerFromCurrent(uint16_t myIndex, uint16_t toIndex, FadeLayer::Interpolator interpolator, uint16_t startDelayMs, uint16_t durationMs) {
    addFadeLayer(m_rootLayer.child(),
                 m_indexedLayers.take(toIndex),
                 myIndex, interpolator, startDelayMs, durationMs);
}

uint16_t LayerController::addFadeLayerFromCurrent(uint16_t toIndex, FadeLayer::Interpolator interpolator, uint16_t startDelayMs, uint16_t durationMs) {
    uint16_t index = getFreeIndex();
